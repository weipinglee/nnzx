(function autoValidate()
{
	...
	function init ()
    {
        for(var i = 0; i < document.forms.length; i++)
        {
            ...
            {
                var e = f.elements[j];
                if(e.type != "text" && e.type!="password" && e.type!='select-one' && e.type!='textarea' && e.type!='hidden') continue;
	...
	function validateOnChange ()
    {
		...
		switch(pattern)
		{
			...
			case 'mobile': pattern = /^1[2|3|4|5|6|7|8|9][0-9]\d{4,8}$/;break;
			...
			case 'area' : pattern = /^[1-9][0-9]{5}$/;break;//地区代码
		}
		...
		var callback = textfield.getAttribute("callback");
		if((empty==null && value=='') || (value!='' && value.search(pattern) == -1))//验证未通过
        {
			showErr(textfield,alt);
        }
		else if(callback !=null && typeof(res=window[callback](textfield))=='string'){//设置了回调函数，若回调返回字符串，则是验证错误信息，其他形式为验证通过
			showErr(textfield,res);
		}
        else
        {
	...
	function validateOnSubmit  ()
    {
        ...
        for(var i = 0; i < this.elements.length; i++)
        {
            ...
            if((e.type == "text" || e.type == "password" || e.type == "select-one" || e.type == "textarea" || e.type == "hidden") && e.getAttribute("pattern") )
            {
				addEvent(e,'change',validateOnChange);
